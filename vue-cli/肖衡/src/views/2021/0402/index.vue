    ...
    <div class="play_wrap" id="player">
      <el-header class="search_bar">
        ...
        <input
          ...
          autocomplete="off"
          v-model="query"
          v-on:keyup.enter="onKeyEnterIpt"
          class="ipt"
        />
      </el-header>
      <el-main class="center_con">
        <!-- 搜索歌曲列表 -->
        ...
        <div class="player_con"  :class="{playing:isPlaying}"> 
          ...
          <img :src="musicImage" class="cover autoRotate" />
        </div>
        <!-- 评论容器 -->
      ...
      <el-footer>
        <div class="audio_con">
          <audio ref="audio" controls autoplay loop class="myaudio" :src="musicUrl" v-on:keyup.space="onKeyEnterSpace" @pause = "onpause" @play = "onplay"></audio>
        </div>
        <div class="video_con" v-show="isShow" style="display: none;">
          <div class="xD" @click="onClickXd">x</div>
...
export default {
  ...
  methods: {
      ...
      if (this.query.trim().length > 0) {
        ...
        axios
          ...
          .then((b) => {
            ...
            // console.log(b);
            if(b.data.code ==200) {
              this.musicList = b.data.result.songs;
              this.query = "";
            }
          });
      }
    ...
    //点击播放
    playMusic(id){
        var audis = this.$refs.audio;
        this.isPlaying = audis.autoplay;
        axios
          .get("https://autumnfish.cn/song/url?id=" + id)
          .then((b) => {
            // console.log(b);
            // console.log(b.data.data[0].code);
            // console.log(b.data.data[0].url);
            if(b.data.data[0].code==200){
                this.musicUrl = b.data.data[0].url;
            }
            //  console.log(id);
            this.playMusicDatails(id);
            this.musicHotWord(id);
          });
    },
    //点击播放mv
    playMV(mvid){
      ...
      audios.pause();
        axios
          ...
          .then((b) => {
            // console.log(b);
            // console.log(b.data.data.code);
            // console.log(b.data.data.url);
            if(b.data.data.code == 200){
                this.mvUrl = b.data.data.url;
                this.isShow = true;
            }
          });
    },
    //歌曲详情
    playMusicDatails(id){
        axios.get("https://autumnfish.cn/song/detail?ids=" + id).then((b)=> {
            // console.log(b);
            // console.log(b.data.code);
            // console.log(b.data.songs[0].al.picUrl);
            if(b.data.code == 200){
                this.musicImage =b.data.songs[0].al.picUrl;
            }
        })
    },
    //获取热门评价
    musicHotWord(id){
        axios.get("https://autumnfish.cn/comment/hot?type=0&id="+id).then((b)=> {
            // console.log(b);
            if(b.data.code == 200){
                this.hotComments = b.data.hotComments;
